...
async function edit(_id, change) {
    const order = await Order.findByIdAndUpdate(_id, change, { new: true });
    if (!order) throw new Error(`Order with id ${_id} not found`);
    return order.populate('products').execPopulate();
}

/**
 * Delete or destroy an order
 */
async function destroy(_id) {
    const result = await Order.findByIdAndDelete(_id);
    if (!result) throw new Error(`Order with id ${_id} not found`);
}
